...
/**
 * two-tool 核心求解入口。
 * 输入：scramble + 各方法组深度限制（EG/TCLL/LS）
 * 输出：多个候选解（inspection / face / alg / subset / color 等）
 *
 * 关键思想：
 * 1) 对 6 个顶色都搜索（白蓝橙绿红黄）；
 * 2) 对每个顶色再遍历多种握持朝向（X/Z + Y spins）；
 * 3) 使用前向+反向的 meet-in-the-middle 搜索得到大量 face 候选；
 * 4) 对每个候选识别 method/subset，再套算法库；
 * 5) 去重、消步、按步数排序后返回。
 */
...
/**
 * 对单个顶色进行搜索。
 * 通过 base orientation + U 轴旋转，覆盖该颜色可能作为“顶色”的多种观察姿态。
 */
const getSolutionsForColor = (state, color, depths) => {

    // 当前 color 的四个贴纸位置，例如 [1, 5, 6, 20]
...
/**
 * 搜索主循环（meet-in-the-middle）。
 * - frontier：从当前状态正向扩展
 * - backNet/backNetFrontier：从目标方法形态反向扩展
 * 当两侧在同一个 sticker 组合相遇，即得到一个可行 face 解。
 */
...
/**
 * 反向网络的初始目标集合。
 * 这里硬编码了 EG/TCLL/LS 的“可接受末状态” sticker 模式。
 * 这一步是 two-tool 能按方法找解的关键之一。
 */
...
/**
 * 检查前向状态是否命中反向网络。
 * 命中后把两段 move 拼接（反向段需取逆）得到完整 face moves。
 */
...
/**
 * 将候选 face 方案落回真实状态并识别方法。
 * 识别出 method 后，再到 METHOD_DATA 中匹配 subset 与算法（含 AUF）。
 */
...
/**
 * 按 method 选择算法表，识别 subset，并生成 preAUF + alg + postAUF。
 * 这里会尝试 ['', 'U', 'U2', "U'"] 四种 AUF 来匹配目标 OLL/PLL wrapper 条件。
 */
